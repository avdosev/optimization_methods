...
def penalty(x0, f, r, z, eps, rest_eq, rest_not_eq):
    xcur = np.array(x0)
    xnew = optimal_gradient_method(lambda x:getAuxilitaryFunctionResult(f, r, rest_eq, rest_not_eq, x), xcur)
    while ((xcur - xnew)**2).sum()>eps:
        r *= z
        xcur, xnew = xnew, optimal_gradient_method(lambda x:getAuxilitaryFunctionResult(f, r, rest_eq, rest_not_eq, x), xcur)
    return xnew
